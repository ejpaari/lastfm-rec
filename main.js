...
function getArtists() {
    ...
    $.ajax({
        ...
        success: function (response) {
            var artists = response.split(',');
            $("#artists").text("Playlist generated from: " + response;
            fetchArtistPlaylist(artists, username);
            console.log(artists);
        },
        error: function () {
            console.log("Something went wrong with LastFM");
        }
    });
}

function fetchArtistPlaylist(artists, username) {
    ...
    $.getJSON(url, { 
        ...
        .done(function(data) {
            ...
            if (! ('songs' in data.response)) {
                ...
                var spotifyPlayButton = getSpotifyPlayButtonForPlaylist("", data.response.songs);
